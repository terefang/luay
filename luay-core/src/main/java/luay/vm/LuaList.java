...
{
	public static LuaList from(LuaValue... values)
	{
		final LuaList _ret = new LuaList();
	...
	public static LuaList from(Object... values)
	{
		LuaValue[] lvalues = new LuaValue[values.length];
		for (int i = 0; i < lvalues.length; i++)
		{
			lvalues[i] = CoerceJavaToLua.coerce(values[i]);
		}
		return from();
	}
